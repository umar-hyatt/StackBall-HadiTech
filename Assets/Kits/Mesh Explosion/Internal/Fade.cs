...
public class Fade : MonoBehaviour {
	...
	static string[] urpStandardShaderNames = new string[] {
		...
		// These are the new names from at least version 7.0.1 of the URP package (as included with Unity 2019.3):
	...
	static bool IsURPStandardShader(Shader shader) {
		return System.Array.IndexOf(urpStandardShaderNames, shader.name) != -1;
	}
	...
	static string[] hdrpStandardShaderNames = new string[] {
		...
		// These are the new names from at least version 5.7.2-preview of the HDRP package (as included with Unity 2019.1):
		"HDRP/LayeredLit",
	...
	public static Shader GetReplacementFor(Shader original) {
		...
		if (IsStandardShader(original) || IsURPStandardShader(original) || IsHDRPStandardShader(original)) {
	...
	IEnumerator Start() {
		...
		foreach (var m in mat) {
			...
			if (IsStandardShader(shader)) {
			...
			else if (IsURPStandardShader(shader)) {
				...
				// The 'Preserve specular lighting' option is usually on by default but we don't want it because we fade out completely and with the option on we still get the full specular contribution even when we want to be almost completely transparent:
				m.DisableKeyword("_BLENDMODE_PRESERVE_SPECULAR_LIGHTING");
				m.SetFloat("_EnableBlendModePreserveSpecularLighting", 0);
				m.EnableKeyword("_ENABLE_FOG_ON_TRANSPARENT");
				m.EnableKeyword("_SURFACE_TYPE_TRANSPARENT");
				m.SetFloat("_AlphaDstBlend", 10);
				m.SetFloat("_DstBlend", 10);
				m.SetFloat("_SurfaceType", 1);
				m.SetFloat("_ZTestDepthEqualForOpaque", 4);
				m.SetFloat("_ZWrite", 0);
				// Enable the "Back Then Front Rendering" option to improve sorting and prevent flickering:
				m.SetShaderPassEnabled("TransparentBackface", true);
				m.SetFloat("_TransparentBackfaceEnable", 1);
			}
#endif
